...
@Component
public class AssetAlertPredicate extends AlertPredicateBuilder {

    @Override
    protected Predicate buildPredicate(From<?, Alert> from, CriteriaBuilder cb, AlertFilterDto filter) {
        return from.get(Alert.ASSET).in(filter.getAssets());
    }

    @Override
    protected boolean canApply(AlertFilterDto filter) {
        return filter.getAssets() != null && !filter.getAssets().isEmpty();
    }
}

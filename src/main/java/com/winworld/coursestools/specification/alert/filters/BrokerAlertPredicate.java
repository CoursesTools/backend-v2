...
@Component
public class BrokerAlertPredicate extends AlertPredicateBuilder {

    @Override
    protected Predicate buildPredicate(From<?, Alert> from, CriteriaBuilder cb, AlertFilterDto filter) {
        return from.get(Alert.BROKER).in(filter.getBrokers());
    }

    @Override
    protected boolean canApply(AlertFilterDto filter) {
        return filter.getBrokers() != null && !filter.getBrokers().isEmpty();
    }
}

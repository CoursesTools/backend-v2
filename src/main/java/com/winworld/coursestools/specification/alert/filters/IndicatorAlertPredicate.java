...
@Component
public class IndicatorAlertPredicate extends AlertPredicateBuilder {

    @Override
    protected Predicate buildPredicate(From<?, Alert> from, CriteriaBuilder cb, AlertFilterDto filter) {
        return from.get(Alert.INDICATOR).in(filter.getIndicators());
    }

    @Override
    protected boolean canApply(AlertFilterDto filter) {
        return filter.getIndicators() != null && !filter.getIndicators().isEmpty();
    }
}

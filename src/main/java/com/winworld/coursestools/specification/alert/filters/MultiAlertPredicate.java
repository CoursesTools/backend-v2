...
@Component
public class MultiAlertPredicate extends AlertPredicateBuilder {

    @Override
    protected Predicate buildPredicate(From<?, Alert> from, CriteriaBuilder cb, AlertFilterDto filter) {
        return cb.equal(from.get(Alert.MULTI_ALERT), filter.getMultiAlert());
    }

    @Override
    protected boolean canApply(AlertFilterDto filter) {
        return filter.getMultiAlert() != null;
    }
}
